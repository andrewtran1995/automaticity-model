...
RSN_C=100; RSN_rv=-60; RSN_vt=-40; k_rsn=0.7;       %RS -- regular spiking
RSN_a=0.03; RSN_b=-2; RSN_c=-50; RSN_d=100;         %RS -- regular spiking
RSN_vpeak=35; RSN_E=60;                             %RS -- regular spiking
...
for j=1:trials
    ...
    v_PFC_A = RSN_rv*ones(1,n) ;     u_PFC_A = zeros(1,n);
    v_PFC_B = RSN_rv*ones(1,n) ;     u_PFC_B = zeros(1,n);
    v_PMC_A = RSN_rv*ones(1,n) ;     u_PMC_A = zeros(1,n);
    v_PMC_B = RSN_rv*ones(1,n) ;     u_PMC_B = zeros(1,n);
    ...
    for i=1:n-1
        ...
        v_PFC_A(i+1)=(v_PFC_A(i) + TAU*(k_rsn*(v_PFC_A(i)-RSN_rv)*(v_PFC_A(i)-RSN_vt)-u_PFC_A(i)+ RSN_E + pfcA_stim + (w_IpmcA*I_PMC_A(i)) - w_pFC_LI*I_PFC_B(i))/RSN_C);   % no noise
        u_PFC_A(i+1)=u_PFC_A(i)+TAU*RSN_a*(RSN_b*(v_PFC_A(i)-RSN_rv)-u_PFC_A(i));
        if v_PFC_A(i+1)>=RSN_vpeak;
            v_PFC_A(i)= RSN_vpeak;
            v_PFC_A(i+1)= RSN_c;
            u_PFC_A(i+1)= u_PFC_A(i+1)+ RSN_d;
        end 
        
        if (v_PFC_A(i) >= RSN_vpeak)
        ...
        v_PFC_B(i+1)=(v_PFC_B(i) + TAU*(k_rsn*(v_PFC_B(i)-RSN_rv)*(v_PFC_B(i)-RSN_vt)-u_PFC_B(i)+ RSN_E + pfcB_stim + (w_IpmcB*I_PMC_B(i)) - w_pFC_LI*I_PFC_A(i))/RSN_C);   % no noise
        u_PFC_B(i+1)=u_PFC_B(i)+TAU*RSN_a*(RSN_b*(v_PFC_B(i)-RSN_rv)-u_PFC_B(i));
        if v_PFC_B(i+1)>=RSN_vpeak;
            v_PFC_B(i)= RSN_vpeak;
            v_PFC_B(i+1)= RSN_c;
            u_PFC_B(i+1)= u_PFC_B(i+1)+ RSN_d;
        end 
        
        if (v_PFC_B(i) >= RSN_vpeak)
        ...
        v_PMC_A(i+1)=(v_PMC_A(i) + TAU*(k_rsn*(v_PMC_A(i)-RSN_rv)*(v_PMC_A(i)-RSN_vt)-u_PMC_A(i)+ RSN_E + (v_stim*full_matrix(r1,r2,1)) + (w_IpfcA*I_PFC_A(i)) - w_PMC_LI*I_PMC_B(i) )/RSN_C);   % no noise
        u_PMC_A(i+1)=u_PMC_A(i)+TAU*RSN_a*(RSN_b*(v_PMC_A(i)-RSN_rv)-u_PMC_A(i));
        if v_PMC_A(i+1)>=RSN_vpeak;
            v_PMC_A(i)= RSN_vpeak;
            v_PMC_A(i+1)= RSN_c;
            u_PMC_A(i+1)= u_PMC_A(i+1)+ RSN_d;
        end 
        
        if (v_PMC_A(i) >= RSN_vpeak)
        ...
        v_PMC_B(i+1)=(v_PMC_B(i) + TAU*(k_rsn*(v_PMC_B(i)-RSN_rv)*(v_PMC_B(i)-RSN_vt)-u_PMC_B(i)+ RSN_E + (v_stim*full_matrix(r1,r2,2)) + (w_IpfcB*I_PFC_B(i)) - w_PMC_LI*I_PMC_A(i) )/RSN_C);   % no noise
        u_PMC_B(i+1)=u_PMC_B(i)+TAU*RSN_a*(RSN_b*(v_PMC_B(i)-RSN_rv)-u_PMC_B(i));
        if v_PMC_B(i+1)>=RSN_vpeak;
            v_PMC_B(i)= RSN_vpeak;
            v_PMC_B(i+1)= RSN_c;
            u_PMC_B(i+1)= u_PMC_B(i+1)+ RSN_d;
        end 
        
        if (v_PMC_B(i) >= RSN_vpeak)
