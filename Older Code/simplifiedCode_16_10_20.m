...
for j=1:trials
    ...
    disp(fprintf('~~~ TRIAL #: %d ~~~', trial_number));
    disp(fprintf('r1: %d', r1));
    disp(fprintf('r2: %d', r2));
    disp(fprintf('v_stim: %d', v_stim));
    disp('full_matrix(r1,r2,1)');
    disp(full_matrix(:,:,1));
    disp('full_matrix(r1,r2,2)');
    disp(full_matrix(:,:,2));
...
set(slider_PMC_A, 'Callback', 'subplot(rows,columns,11);PMC_A_trial_num=nearest(get(slider_PMC_A,''value''));set(slider_PMC_A, ''Value'', PMC_A_trial_num);data3 = PMC_A.weights(:,:,PMC_A_trial_num);colormap(''hot'');imagesc(data3);colorbar;title(sprintf(''PMC_A Synaptic Heatmap, Trial %d\n'', PMC_A_trial_num));');
...
slider_PMC_B = uicontrol('Style', 'slider', ...
    'Min', 1, 'Max', trials, ...
    'Value', 1, ...
    'Position', [1500 50 400 20]);
set(slider_PMC_B, 'Callback', 'subplot(rows,columns,12);PMC_B_trial_num=nearest(get(slider_PMC_B,''value''));set(slider_PMC_B, ''Value'', PMC_B_trial_num);data4 = PMC_B.weights(:,:,PMC_B_trial_num);colormap(''hot'');imagesc(data4);colorbar;title(sprintf(''PMC_B Synaptic Heatmap, Trial %d\n'', PMC_B_trial_num));');
