...
TAU = 1;
TRIALS = 500;              % Number of trials in automaticity experiment
...
set(slider_PMC_A, 'Callback', 'subplot(rows,columns,1,''replace'');PMC_A_trial_num=round(get(slider_PMC_A,''value''));set(slider_PMC_A, ''Value'', PMC_A_trial_num);data3 = PMC_A.weights(:,:,PMC_A_trial_num);colormap(''hot'');imagesc(data3);colorbar;title(sprintf(''PMC_A Synaptic Heatmap, Trial %d\n'', PMC_A_trial_num));');
...
set(slider_PMC_B, 'Callback', 'subplot(rows,columns,2,''replace'');PMC_B_trial_num=round(get(slider_PMC_B,''value''));set(slider_PMC_B, ''Value'', PMC_B_trial_num);data4 = PMC_B.weights(:,:,PMC_B_trial_num);colormap(''hot'');imagesc(data4);colorbar;title(sprintf(''PMC_B Synaptic Heatmap, Trial %d\n'', PMC_B_trial_num));');
