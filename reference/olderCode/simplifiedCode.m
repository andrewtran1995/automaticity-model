...
%PFC A Neuron
    ...
    v_PFC_A(i+1)=(v_PFC_A(i) + tau*(k_rsn*(v_PFC_A(i)-vr_rsn)*(v_PFC_A(i)-vt_rsn)-u_PFC_A(i)+ E_rsn + pfcA_stim + (w_IpmcA*I_PMC_A(i)) - w_pFC_LI*I_PFC_B(i))/C_rsn);   % no noise
...
%PFC B Neuron
    ...
    v_PFC_B(i+1)=(v_PFC_B(i) + tau*(k_rsn*(v_PFC_B(i)-vr_rsn)*(v_PFC_B(i)-vt_rsn)-u_PFC_B(i)+ E_rsn + pfcB_stim + (w_IpmcB*I_PMC_B(i)) - w_pFC_LI*I_PFC_A(i))/C_rsn);   % no noise
...
%PMC_A Neuron
    ...
    v_PMC_A(i+1)=(v_PMC_A(i) + tau*(k_rsn*(v_PMC_A(i)-vr_rsn)*(v_PMC_A(i)-vt_rsn)-u_PMC_A(i)+ E_rsn + (v_stim*full_matrix(r1,r2,1)) + (w_IpfcA*I_PFC_A(i)) - w_PMC_LI*I_PMC_B(i) )/C_rsn);   % no noise
...
%PMC_B Neuron
    ...
    v_PMC_B(i+1)=(v_PMC_B(i) + tau*(k_rsn*(v_PMC_B(i)-vr_rsn)*(v_PMC_B(i)-vt_rsn)-u_PMC_B(i)+ E_rsn + (v_stim*full_matrix(r1,r2,2)) + (w_IpfcB*I_PFC_B(i)) - w_PMC_LI*I_PMC_A(i) )/C_rsn);   % no noise
